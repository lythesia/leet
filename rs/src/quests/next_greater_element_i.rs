...
impl Solution {
    pub fn next_greater_element(nums1: Vec<i32>, nums2: Vec<i32>) -> Vec<i32> {
        let mut next = [-1; 10001];
        let mut mono_stack = vec![];
        // iter backwards
        for x in nums2.into_iter().rev() {
            while let Some(&top) = mono_stack.last() {
                // pop until x < top
                if x >= top { mono_stack.pop(); }
                else { break; }
            }
            next[x as usize] = *mono_stack.last().unwrap_or(&-1);
            // push
            mono_stack.push(x);
        }
        nums1.into_iter().map(|x| next[x as usize]).collect()
    }
}
